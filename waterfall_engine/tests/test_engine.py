...
import unittest
from waterfall_engine.engine import Deal, WaterfallEngine
...
from waterfall_engine.models import RawPaymentContext, WaterfallProcessor
...
class TestEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        revenue_waterfall_limbs = {
            ...
            4: WaterfallProcessor(tranche_b),
        }
    ...
    def test_revenue_waterfall(self):
        ...
        WaterfallEngine(self.my_deal).run_all_IPDs(payment_context)
        ...
        for result in results:
            for period, data in result.items():
                ...
                self.assertAlmostEqual(data['surplus']['amount_paid'], 99750.0)
    ...
    def test_pro_rata_redemption_waterfall(self):
        ...
        for result in results:
            for period, data in result.items():
                ...
                self.assertAlmostEqual(
                    data['surplus']['amount_paid'], 0.0,
                    msg=f"Period {period} - Error in excess spread redemption: {data['surplus']['amount_paid']}"
                )
